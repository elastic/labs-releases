...
def main() -> None:
    ...
    if not args.output and not args.elastic and not args.local:
        ...
        if args.elastic or args.local:
            ...
            if args.configuration:
                cloud_id, api_key, index, auth_or_url, use_cloud = load_configuration(pathlib.Path(args.configuration), use_cloud)
                
                if use_cloud:
                    url = None  # URL not needed for cloud
                    auth = None  # Authentication not needed for cloud
                else:
                    url = auth_or_url  # Use URL for local
                    auth = f"{auth_or_url}"  # Authentication for local
